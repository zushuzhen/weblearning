...
const { active, content, partsId } = toRefs(pageData);
...
const getthree1 = (result) => {
  gettree(result, false).then((res) => {
    ...
    if (partsId.value) {
      active.value = res.data.findIndex((item) => item.id === partsId.value);
      console.log(active.value);
      if(active.value==-1){
        active.value=0
      getDetail(res.data[0].id);
      }
      getDetail(partsId.value);
    } else {
      getDetail(res.data[0].id);
    }
  });
...
onMounted(() => {
  if (props.cmpData.url == location.hash) {
    let parts = getQueryString("query");
    if (parts) partsId.value = parts;
  }
...
<template>
  <div class="city">
    <div class="consulting">
      ...
      <div class="con-right">
        <div class="boxtitle">
          <div class="box-top">{{ content.title }}</div>
          <div class="box-bottom"></div>
        </div>
        <p v-html="content.content"></p>
        <div
          class="boxImg"
          v-if="content && content.img && content.img.length > 0"
        >
          <img v-for="item in content.img" :key="item" :src="item.url" alt="" />
        </div>
        <div class="vr" v-if="content.jumpurl" @click="vrbtn(content.jumpurl)"> <i class="iconfont">&#xe64a;</i><span style="text-decoration: underline;">查看VR</span></div>
      </div>
    </div>
  </div>
</template>
